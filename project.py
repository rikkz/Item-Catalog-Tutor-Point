...
def createUser(login_session):
    ...
    session.add(newUser)
    session.commit()
    user = session.query(User).filter_by(email=login_session['em' +
                                         'ail']).one()
    return user.id
...
def gconnect():
    ...
    if result['user_id'] != gplus_id:
        response = \
            make_response(json.dumps("Token's user I" +
                                     "D doesn't match given user ID."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def specificStudentJSON(tutor_id, student_id):
    items = session.query(Student).filter_by(tutor_id=tutor_id,
                                             id=student_id).all()
    return jsonify(Items=[i.serialize for i in items])
...
@app.route('/tutor/<int:tutor_id>/student/<int:student_id>/edit',
           methods=['GET', 'POST'])
def editTutorStudent(tutor_id, student_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editStudent.html', tutor_id=tutor_id,
                               i=nq, log=login_session)


@app.route('/tutor/<int:tutor_id>/student/<int:student_id>/delete',
           methods=['GET', 'POST'])
